 ...
 *
 * File structure mirrors pilot-voice-agent-frontend-ppt_update's PPTView.tsx
 * exactly (its active PPTCopilotView component, unmodified) — the 3
 * previously-re-ported PILOT-specific features (draw-to-place text box,
 * SlideCanvas-as-sibling layout, Frame wrapper, and the fullscreen Presenter
 * View) are intentionally NOT included, per explicit request to match the
 * reference wholesale. PPTPageView (session wiring) is appended at the end,
 * consolidating what previously lived in transcript/PPTPageView.tsx.
 */
...
import { useSession } from "./transcript/useSession";
import { isPptRelated } from "./transcript/helpers";
import { LiveTranscriptBar } from "./transcript/LiveTranscriptBar";
          ...
          <div style={{ background:C.surface, borderRadius:16, padding:"1.75rem",
            ...
            <textarea
              ...
              style={{ width:"100%", boxSizing:"border-box", padding:"0.55rem 0.75rem",
                       ...
                       resize:"none", maxHeight:60, overflowY:"auto",
                       fontFamily:"inherit", outline:"none",
                       marginBottom:"1.4rem" }}
            />
...
/* ── PPT page view — session wiring (transcript filtering, live-transcript
   bar, mic toggle) around PPTCopilotView above. Previously lived in its own
   file (transcript/PPTPageView.tsx) as a thin wrapper; consolidated here so
   everything PPT-view-related lives in one file. ── */
export function PPTPageView() {
  const sess = useSession();
  const pptTranscripts = sess.transcripts.filter(t => isPptRelated(t.text));

  return (
    // Scrolls as ONE section (navbar down to the mic bar) if content ever needs
    // more height than the viewport, instead of clipping/squishing internals.
